...
import { HANDLED, HttpError, badRequest, conflict, forbidden, isLocalRequest, notFound, rateLimit, readBody, readJson, requireAuth, route, send, unauthorized, v, } from "./http.js";
import { instanceHealth, invalidateStorageCache, VERSION } from "./instance.js";
import { CDN_REENVIABLE, deleteAttachmentsOf, deleteAttachmentsOwnedBy, linkAttachments, purgeChatFiles, saveRemoteAttachment, saveUpload, serveFile } from "./storage.js";
...
function requirePerm(communityId, userId, perm, what) {
    if (!has(communityPermissions(communityId, userId), perm))
        throw forbidden(`Te falta el permiso para ${what}.`);
}
function requireChannelPerm(channelId, userId, perm, what) {
    if (!has(channelPermissions(channelId, userId), perm))
        throw forbidden(`Te falta el permiso para ${what}.`);
}
function requireMembership(communityId, userId) {
    const state = memberState(communityId, userId);
    if (!state.isMember || state.banned)
        throw notFound("Comunidad no encontrada.");
}
...
route("POST", "/api/v1/auth/recover", async (ctx) => {
    ...
    if (!user || user.password_hash)
        throw unauthorized("No hay ninguna cuenta sin contraseña con ese nombre.");
...
route("POST", "/api/v1/auth/bootstrap", async (ctx) => {
    if (countOwners() > 0)
        throw conflict("Esta instancia ya tiene dueño. Entra con tu cuenta.");
    ...
    if (!USERNAME.test(username))
        throw badRequest("Ese nombre de usuario no es válido.");
...
route("POST", "/api/v1/auth/guest", async (ctx) => {
    if (!config.guestModeEnabled)
        throw forbidden("Esta instancia no admite invitados.");
...
route("POST", "/api/v1/auth/refresh", async (ctx) => {
    ...
    if (!session)
        throw unauthorized("Sesión caducada, vuelve a entrar.");
    const user = findUserById(db.prepare("SELECT user_id FROM sessions WHERE id = ?").get(session.sessionId).user_id);
    return {
        access_token: session.accessToken,
        refresh_token: session.refreshToken,
        expires_in: session.expiresIn,
...
route("POST", "/api/v1/auth/logout", (ctx) => {
    ...
    if (header?.startsWith("Bearer ")) {
        ...
        if (auth)
            disconnectSession(auth.sessionId);
        revokeSession(token);
    }
});
/* ── perfil (§10.1) ────────────────────────────────────────────────── */
...
route("PATCH", "/api/v1/users/me", async (ctx) => {
    ...
    for (const key of ["bio", "pronouns", "avatar_url", "banner_url"]) {
    ...
    if (body.locale !== undefined)
        fields.push(["locale", v.oneOf(body, "locale", ["es", "pt-BR", "en"])]);
    if (body.theme !== undefined)
        fields.push(["theme", v.oneOf(body, "theme", ["light", "dark", "system"])]);
    if (body.status !== undefined)
        fields.push(["status", v.oneOf(body, "status", USER_STATUSES)]);
    ...
    if (body.settings !== undefined) {
        if (typeof body.settings !== "object" || body.settings === null)
            throw badRequest('"settings" debe ser un objeto.');
    ...
    db.prepare(`UPDATE users SET ${fields.map(([k]) => `${k} = ?`).join(", ")} WHERE id = ?`).run(...fields.map(([, value]) => value), user.id);
    ...
    if (body.settings !== undefined && !sharesGameActivity(user.id))
        clearPlaying(user.id);
    for (const community of communitiesForUser(user.id)) {
        const member = getMember(community.id, user.id);
        if (member)
            publish(community.id, { t: "MEMBER_UPDATE", d: member });
...
route("PUT", "/api/v1/users/me/game-presence", async (ctx) => {
    ...
    if (!sharesGameActivity(user.id))
        throw forbidden("La actividad de juego está desactivada en tus ajustes.");
    ...
    if (setPlaying(user.id, gameName))
        announceGame(user.id);
});
route("DELETE", "/api/v1/users/me/game-presence", (ctx) => {
    const { user } = requireAuth(ctx);
    if (clearPlaying(user.id))
        announceGame(user.id);
});
...
route("POST", "/api/v1/users/me/upgrade", async (ctx) => {
    ...
    if (current.password_hash)
        throw conflict("Esta cuenta ya tiene contraseña.");
    ...
    if (taken && taken.id !== user.id)
        throw conflict("Ese nombre de usuario ya existe.");
    db.prepare("UPDATE users SET username = ?, password_hash = ?, kind = 'local' WHERE id = ?").run(username, hashPassword(password), user.id);
    return toSelfUser(findUserById(user.id));
});
/**
...
route("POST", "/api/v1/users/me/password", async (ctx) => {
    ...
    const current = findUserById(user.id);
    if (!current.password_hash)
        throw conflict("Esta cuenta no tiene contraseña todavía. Ponla desde «convertir en cuenta permanente».");
...
route("DELETE", "/api/v1/users/me", async (ctx) => {
    ...
    const owned = db.prepare("SELECT id FROM communities WHERE owner_id = ?").all(user.id);
...
route("GET", "/api/v1/communities/:id/bootstrap", (ctx) => {
    ...
    const visible = channelsOf(communityId).filter((channel) => has(channelPermissions(channel.id, user.id), PERMISSIONS.VIEW_CHANNEL));
    ...
    for (const channel of visible)
        channelPerms[channel.id] = channelPermissions(channel.id, user.id).toString();
    return {
        ...
        read_state: Object.fromEntries(visible.map((channel) => [channel.id, lastReadId(user.id, channel.id)]).filter(([, id]) => id !== null)),
    };
});
route("PATCH", "/api/v1/communities/:id", async (ctx) => {
    ...
    for (const key of ["description", "icon_url", "banner_url", "rules"]) {
    ...
    if (body.is_public !== undefined)
        fields.push(["is_public", v.bool(body, "is_public", false) ? 1 : 0]);
    if (fields.length === 0)
        return getCommunity(communityId);
    db.prepare(`UPDATE communities SET ${fields.map(([k]) => `${k} = ?`).join(", ")} WHERE id = ?`).run(...fields.map(([, value]) => value), communityId);
...
route("DELETE", "/api/v1/communities/:id", (ctx) => {
    ...
    if (community.owner_id !== user.id)
        throw forbidden("Solo quien creó la comunidad puede eliminarla.");
    deleteExpressionAttachmentsOfCommunity(community.id);
    db.prepare("DELETE FROM communities WHERE id = ?").run(community.id);
    publish(community.id, { t: "MEMBER_LEAVE", d: { community_id: community.id, user_id: user.id } });
});
route("POST", "/api/v1/communities/:id/leave", (ctx) => {
    ...
    if (!community)
        throw notFound("Comunidad no encontrada.");
    if (community.owner_id === user.id)
        throw conflict("Transfiere o elimina la comunidad antes de salir.");
...
route("GET", "/api/v1/discovery", (ctx) => {
    if (!config.publicDiscoveryEnabled)
        return [];
    const rows = db
        .prepare(`SELECT c.id, c.name, c.slug, c.description, c.icon_url, c.banner_url, c.accent_color,
...
route("POST", "/api/v1/communities/:id/categories", async (ctx) => {
    ...
    const id = uuidv7();
    db.prepare("INSERT INTO categories (id, community_id, name, position) VALUES (?, ?, ?, ?)").run(id, communityId, name, position);
...
route("PATCH", "/api/v1/categories/:id", async (ctx) => {
    const { user } = requireAuth(ctx);
    const row = db.prepare("SELECT community_id FROM categories WHERE id = ?").get(ctx.params.id);
    if (!row)
    ...
    const name = v.optionalString(body, "name", { max: 64 });
    if (name)
        db.prepare("UPDATE categories SET name = ? WHERE id = ?").run(name, ctx.params.id);
    if (body.position !== undefined)
        db.prepare("UPDATE categories SET position = ? WHERE id = ?").run(v.int(body, "position", { min: 0, max: 1000 }), ctx.params.id);
...
route("POST", "/api/v1/communities/:id/channels", async (ctx) => {
    ...
    const kind = v.oneOf(body, "kind", ["text", "voice", "announcement"], "text");
    ...
    db.prepare(`INSERT INTO channels (id, community_id, category_id, name, topic, kind, position, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, communityId, categoryId, name, topic, kind, channelsOf(communityId).length, Date.now());
...
route("PATCH", "/api/v1/channels/:id", async (ctx) => {
    ...
    if (topic !== undefined)
        fields.push(["topic", topic || null]);
    if (body.category_id !== undefined)
        fields.push(["category_id", v.optionalString(body, "category_id", { max: 64 }) || null]);
    if (body.position !== undefined)
        fields.push(["position", v.int(body, "position", { min: 0, max: 1000 })]);
    if (body.slowmode_s !== undefined)
        fields.push(["slowmode_s", v.int(body, "slowmode_s", { min: 0, max: 21600 })]);
    if (fields.length === 0)
        return channel;
    db.prepare(`UPDATE channels SET ${fields.map(([k]) => `${k} = ?`).join(", ")} WHERE id = ?`).run(...fields.map(([, value]) => value), channel.id);
...
route("PUT", "/api/v1/channels/:id/permissions/:targetId", async (ctx) => {
    ...
    const body = await readJson(ctx);
    const targetType = v.oneOf(body, "target_type", ["role", "member"]);
    ...
    db.prepare(`INSERT INTO overwrites (channel_id, target_id, target_type, allow, deny) VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(channel_id, target_id) DO UPDATE SET allow = excluded.allow, deny = excluded.deny, target_type = excluded.target_type`).run(channel.id, ctx.params.targetId, targetType, allow.toString(), deny.toString());
...
route("POST", "/api/v1/channels/:id/messages", async (ctx) => {
    ...
    const attachmentIds = Array.isArray(body.attachment_ids)
        ? body.attachment_ids.filter((id) => typeof id === "string").slice(0, 10)
        : [];
    ...
    const mentionsEveryone = /(^|\s)@(everyone|todos)\b/.test(content) &&
        has(channelPermissions(channel.id, user.id), PERMISSIONS.MENTION_EVERYONE);
    const id = uuidv7();
    db.prepare(`INSERT INTO messages (id, channel_id, community_id, author_id, content, created_at, reply_to_id, mentions_everyone)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, channel.id, channel.community_id, user.id, content, Date.now(), replyTo, mentionsEveryone ? 1 : 0);
...
route("PATCH", "/api/v1/messages/:id", async (ctx) => {
    ...
    if (message.author_id !== user.id)
        throw forbidden("Solo puedes editar tus propios mensajes.");
    const body = await readJson(ctx);
...
route("DELETE", "/api/v1/messages/:id", (ctx) => {
    ...
    if (!isAuthor)
        requireChannelPerm(message.channel_id, user.id, PERMISSIONS.MANAGE_MESSAGES, "borrar mensajes ajenos");
    deleteAttachmentsOf(message.id);
    db.prepare("DELETE FROM messages WHERE id = ?").run(message.id);
    if (!isAuthor)
        audit(message.community_id, user.id, "MESSAGE_DELETE", message.id, { author_id: message.author_id });
...
route("GET", "/api/v1/channels/:id/pins", (ctx) => {
    ...
    const rows = db.prepare("SELECT id FROM messages WHERE channel_id = ? AND pinned = 1 ORDER BY id DESC").all(ctx.params.id);
...
route("POST", "/api/v1/channels/:id/read", async (ctx) => {
    ...
    db.prepare(`INSERT INTO read_state (user_id, channel_id, last_read_id, updated_at) VALUES (?, ?, ?, ?)
     ON CONFLICT(user_id, channel_id) DO UPDATE SET last_read_id = excluded.last_read_id, updated_at = excluded.updated_at`).run(user.id, channelId, furthest, Date.now());
...
route("GET", "/api/v1/channels/:id/search", (ctx) => {
    ...
    const rows = db
        ...
        .all(ctx.params.id, `%${q.replace(/[%_\\]/g, "\\$&")}%`);
    return rows.map((r) => getMessage(r.id)).filter(Boolean);
});
route("POST", "/api/v1/messages/:id/reactions", async (ctx) => {
    ...
    db.prepare("INSERT OR IGNORE INTO reactions (message_id, user_id, emoji) VALUES (?, ?, ?)").run(message.id, user.id, emoji);
    const updated = getMessage(message.id);
    publishToChannel(message.community_id, message.channel_id, {
        t: "REACTION_UPDATE",
...
route("DELETE", "/api/v1/messages/:id/reactions", (ctx) => {
    ...
    if (!emoji)
        throw badRequest("Falta el parámetro emoji.");
...
route("POST", "/api/v1/communities/:id/roles", async (ctx) => {
    ...
    const id = uuidv7();
    db.prepare("INSERT INTO roles (id, community_id, name, color, permissions, position, hoist, mentionable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(id, communityId, name, color, permissions.toString(), position, v.bool(body, "hoist", false) ? 1 : 0, v.bool(body, "mentionable", true) ? 1 : 0);
...
function grantablePermissions(communityId, userId, body) {
    ...
    if (has(own, PERMISSIONS.ADMINISTRATOR))
        return requested & ALL_PERMISSIONS;
    const excess = requested & ~own;
    if (excess !== 0n)
        throw forbidden("No puedes conceder permisos que tú no tienes.");
    return requested & ALL_PERMISSIONS;
}
route("PATCH", "/api/v1/roles/:id", async (ctx) => {
    ...
    if (name && !role.is_default)
        fields.push(["name", name]);
    const color = v.color(body, "color");
    if (color !== undefined)
        fields.push(["color", color]);
    if (body.permissions !== undefined)
        fields.push(["permissions", grantablePermissions(role.community_id, user.id, body).toString()]);
    if (body.hoist !== undefined)
        fields.push(["hoist", v.bool(body, "hoist", false) ? 1 : 0]);
    if (body.mentionable !== undefined)
        fields.push(["mentionable", v.bool(body, "mentionable", true) ? 1 : 0]);
    ...
    if (fields.length === 0)
        return role;
    db.prepare(`UPDATE roles SET ${fields.map(([k]) => `${k} = ?`).join(", ")} WHERE id = ?`).run(...fields.map(([, value]) => value), role.id);
...
route("DELETE", "/api/v1/roles/:id", (ctx) => {
    ...
    if (!role)
        throw notFound("Rol no encontrado.");
    if (role.is_default)
        throw conflict("El rol base no se puede eliminar.");
...
route("PATCH", "/api/v1/communities/:id/members/:userId", async (ctx) => {
    ...
    requireMembership(communityId, user.id);
    if (!getMember(communityId, targetId))
        throw notFound("Ese miembro no está en la comunidad.");
    const body = await readJson(ctx);
    const isSelf = targetId === user.id;
    if (body.nickname !== undefined) {
        if (!isSelf)
            requirePerm(communityId, user.id, PERMISSIONS.MANAGE_MEMBERS, "cambiar apodos");
        db.prepare("UPDATE members SET nickname = ? WHERE community_id = ? AND user_id = ?").run(v.optionalString(body, "nickname", { max: 48 }) || null, communityId, targetId);
    }
    if (body.role_ids !== undefined) {
        requirePerm(communityId, user.id, PERMISSIONS.MANAGE_ROLES, "asignar roles");
        if (!canActOn(communityId, user.id, targetId))
            throw forbidden("Ese miembro está por encima de ti en la jerarquía.");
        if (!Array.isArray(body.role_ids))
            throw badRequest('"role_ids" debe ser una lista.');
        const ceiling = highestRolePosition(communityId, user.id);
        const ids = body.role_ids.filter((id) => typeof id === "string");
        for (const roleId of ids) {
            const role = getRole(roleId);
            if (!role || role.community_id !== communityId)
                throw badRequest("Rol inexistente en esta comunidad.");
            if (role.position >= ceiling)
                throw forbidden(`No puedes asignar el rol "${role.name}".`);
        }
        ...
        for (const roleId of ids)
            add.run(communityId, targetId, roleId);
...
route("GET", "/api/v1/communities/:id/audit", (ctx) => {
    ...
    const rows = db.prepare("SELECT * FROM audit_log WHERE community_id = ? ORDER BY id DESC LIMIT 100").all(communityId);
...
function requireTelegram() {
    if (!config.telegramBotToken)
        throw notFound("Esta instancia no tiene activada la importación de Telegram.");
...
async function telegramFile(fileId) {
    ...
    if (!path)
        throw new HttpError(502, "UPSTREAM_ERROR", "Telegram no encontró ese sticker.");
    const res = await fetch(`https://api.telegram.org/file/bot${token}/${path}`, { signal: corte }).catch(() => null);
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "No se pudo traer el sticker.");
...
route("GET", "/api/v1/stickers", async (ctx) => {
    ...
    if (!/^[a-zA-Z0-9_]{1,64}$/.test(pack))
        throw badRequest("Nombre de paquete no válido.");
    const res = await fetch(`https://api.telegram.org/bot${token}/getStickerSet?name=${encodeURIComponent(pack)}`, {
        signal: AbortSignal.timeout(8000),
    }).catch(() => null);
    const json = (await res?.json().catch(() => null));
    if (!res?.ok || !json?.ok || !json.result)
        throw notFound("No se encontró ese paquete de stickers.");
    const stickers = (json.result.stickers ?? []).flatMap((raw) => {
        const item = raw;
        if (!item.file_id)
            return [];
...
route("GET", "/api/v1/stickers/image", async (ctx) => {
    ...
    if (!/^[\w-]{1,200}$/.test(fileId))
        throw badRequest("Sticker no válido.");
    const { data, contentType } = await telegramFile(fileId);
    if (!contentType.startsWith("image/"))
        throw new HttpError(415, "UNSUPPORTED_MEDIA_TYPE", "Ese sticker no es una imagen estática.");
...
async function readSoundBody(response) {
    if (!response.body)
...
route("POST", "/api/v1/communities/:id/emojis/import-sound", async (ctx) => {
    ...
    if (!MYINSTANTS_MEDIA.test(origen))
        throw badRequest("Ese sonido no viene de la galería.");
    const res = await fetch(origen, { signal: AbortSignal.timeout(8000) }).catch(() => null);
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "No se pudo traer el sonido.");
    ...
    if (data.length === 0)
        throw new HttpError(415, "UNSUPPORTED_MEDIA_TYPE", "El sonido está vacío.");
...
async function askGiphy(recurso, path, params) {
    if (!config.giphyApiKey)
        throw notFound("Esta instancia no tiene el buscador de GIF activado.");
    ...
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "Giphy no respondió. Prueba otra vez en un momento.");
    const json = (await res.json());
    const lista = Array.isArray(json.data) ? json.data : [];
    return lista.flatMap((raw) => {
        const item = raw;
        ...
        if (!item.id || !grande?.url || !pequeno?.url)
            return [];
        return [
            {
                id: item.id,
 ...
 */
async function askKlipy(recurso, path, params) {
    if (!config.klipyApiKey)
        throw notFound(`Esta instancia no tiene la galeria de ${recurso} activada.`);
    ...
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "Klipy no respondio. Prueba otra vez en un momento.");
    const json = (await res.json());
    ...
    return lista.flatMap((raw) => {
        ...
        if (!item.slug || !grande?.url || !pequeno?.url)
            return [];
        return [
            {
                id: item.slug,
...
route("GET", "/api/v1/wallpapers", async (ctx) => {
    ...
    if (config.wallhavenApiKey)
        url.searchParams.set("apikey", config.wallhavenApiKey);
    const corte = AbortSignal.timeout(8000);
    const res = await fetch(url, { signal: corte }).catch(() => null);
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "Wallhaven no respondió. Prueba otra vez en un momento.");
    const json = (await res.json());
    const lista = Array.isArray(json.data) ? json.data : [];
    return lista.flatMap((raw) => {
        const item = raw;
        if (!item.id || !item.path || !item.thumbs?.small)
            return [];
...
route("GET", "/api/v1/sounds", async (ctx) => {
    ...
    if (consulta)
        url.searchParams.set("name", consulta);
    url.searchParams.set("page", String(pagina));
    const res = await fetch(url, { signal: AbortSignal.timeout(8000) }).catch(() => null);
    if (!res?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "MyInstants no respondió. Prueba otra vez en un momento.");
    const json = (await res.json().catch(() => null));
    ...
    return lista.flatMap((raw) => {
        ...
        if (!item.slug || !item.name || !item.sound || !MYINSTANTS_MEDIA.test(item.sound))
            return [];
...
route("GET", "/api/v1/avatars/image", async (ctx) => {
    ...
    if (origen.protocol !== "https:" || origen.hostname !== GALERIA)
        throw badRequest("Ese enlace no es de la galería.");
    ...
    if (!res?.ok || !tipo.startsWith("image/"))
        throw new HttpError(502, "UPSTREAM_ERROR", "La imagen no se pudo traer.");
    ctx.res.writeHead(200, {
        "content-type": tipo,
...
route("GET", "/api/v1/avatars", async (ctx) => {
    ...
    if (!/^[a-z]{1,20}$/.test(categoria))
        throw badRequest("Categoría no válida.");
    const cantidad = Math.min(Math.max(Number(ctx.url.searchParams.get("amount") ?? 12) || 12, 1), 20);
    const json = (await nekos(`${categoria}?amount=${cantidad}`));
    const lista = Array.isArray(json.results) ? json.results : [];
    return lista.flatMap((raw) => {
        const item = raw;
        if (!item.url)
...
route("POST", "/api/v1/gifs/save", async (ctx) => {
    ...
    if (!head?.ok)
        throw new HttpError(502, "UPSTREAM_ERROR", "No se pudo comprobar el archivo.");
...
route("POST", "/api/v1/communities/:id/invites", async (ctx) => {
    ...
    db.prepare("INSERT INTO invites (code, community_id, channel_id, creator_id, max_uses, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(code, communityId, channelId, user.id, maxUses, ttlS ? Date.now() + ttlS * 1000 : null, Date.now());
...
route("DELETE", "/api/v1/invites/:code", (ctx) => {
    const { user } = requireAuth(ctx);
    const invite = db.prepare("SELECT * FROM invites WHERE code = ?").get(ctx.params.code);
    if (!invite)
        throw notFound("Invitación no encontrada.");
    if (invite.creator_id !== user.id)
        requirePerm(invite.community_id, user.id, PERMISSIONS.MANAGE_INVITES, "borrar invitaciones");
...
function liveInvite(code) {
    ...
    if (!invite)
        throw notFound("Esa invitación no existe o fue revocada.");
    if (invite.expires_at !== null && invite.expires_at < Date.now())
        throw notFound("Esa invitación caducó.");
    if (invite.max_uses !== null && invite.uses >= invite.max_uses)
        throw notFound("Esa invitación agotó sus usos.");
    return invite;
}
...
route("GET", "/api/v1/invites/:code", (ctx) => {
    ...
    const members = db.prepare("SELECT COUNT(*) AS n FROM members WHERE community_id = ? AND banned = 0").get(invite.community_id).n;
    return {
        code: invite.code,
        community: {
            id: community.id,
...
route("POST", "/api/v1/invites/:code/join", (ctx) => {
    ...
    if (existing.banned)
        throw forbidden("Tienes el acceso bloqueado en esta comunidad.");
    if (!existing.isMember) {
        db.prepare("INSERT INTO members (community_id, user_id, joined_at) VALUES (?, ?, ?)").run(invite.community_id, user.id, Date.now());
...
function requireHost(ctx) {
    ...
    if (!isInstanceOwner(auth.user.id))
...
route("POST", "/api/v1/uploads", async (ctx) => {
    ...
    if (!contentType)
        throw badRequest("Falta la cabecera content-type.");
...
route("GET", "/api/v1/communities/:id/export", (ctx) => {
    ...
    const bundle = {
        ...
        messages,
        attachments: db
            .prepare(`SELECT a.id, a.filename, a.content_type, a.size, a.message_id FROM attachments a
