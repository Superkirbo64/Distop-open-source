...
import { authenticate, findUserById } from "./auth.js";
import { communitiesForUser, getChannel } from "./entities.js";
import { channelPermissions, memberState } from "./permissions.js";
import { instanceHealth } from "./instance.js";
import { rateLimit } from "./http.js";
import * as voice from "./voice.js";
import * as race from "./race.js";
import { getEmoji } from "./expressions.js";
/** Lo que el cliente puede pedir sobre otra persona en una sala. */
const VOICE_ACTIONS = ["mute", "unmute", "deafen", "undeafen", "disconnect"];
 ...
 */
export function onlineIn(communityId) {
    const ids = new Set();
    for (const client of clients)
        if (client.subs.has(communityId))
...
function rejectVoiceSound(client, channelId, soundId, reason) {
...
export function publish(communityId, event) {
    for (const client of clients)
        if (client.subs.has(communityId))
            send(client, event);
}
/** Emite solo a quien puede ver ese canal. */
export function publishToChannel(communityId, channelId, event) {
    for (const client of clients) {
        if (!client.subs.has(communityId))
            continue;
...
export function disconnectSession(sessionId) {
    for (const client of clients)
        if (client.sessionId === sessionId)
            client.ws.close(4001, "sesión revocada");
    ...
    switch (cmd.t) {
        case "SUBSCRIBE": {
            ...
            if (typeof communityId !== "string")
                return;
            if (!memberState(communityId, client.userId).isMember)
                return;
        ...
        case "UNSUBSCRIBE": {
            const communityId = cmd.d?.community_id;
            if (typeof communityId !== "string" || !client.subs.delete(communityId))
                return;
            broadcastPresence(communityId);
            return;
        }
        case "TYPING": {
            ...
            if (!has(channelPermissions(channelId, client.userId), PERMISSIONS.SEND_MESSAGES))
                return;
        ...
        case "VOICE_JOIN": {
            ...
            if (result.left) {
                ...
                if (race.leave(result.left, client.userId))
                    announceRace(result.left);
            }
            announceVoice(channelId);
            // Quien acaba de entrar necesita saber si aquí hay una carrera esperando.
            if (race.lobbyOf(channelId))
                announceRace(channelId);
            return;
        }
        case "VOICE_LEAVE": {
            const channelId = cmd.d?.channel_id;
            if (typeof channelId !== "string")
                return;
            if (voice.leave(channelId, client.userId))
                announceVoice(channelId);
            if (race.leave(channelId, client.userId))
                announceRace(channelId);
            return;
        }
        case "VOICE_MUTE": {
            ...
            if (typeof channelId !== "string")
                return;
            if (voice.setMute(channelId, client.userId, Boolean(muted), Boolean(deafened)))
                announceVoice(channelId);
            return;
        }
        case "VOICE_VIDEO": {
            ...
            if (typeof channelId !== "string")
                return;
            if (source !== null && source !== "camera" && source !== "screen")
                return;
            if (voice.setVideo(channelId, client.userId, source))
                announceVoice(channelId);
            return;
        }
        case "VOICE_MODERATE": {
            const { channel_id: channelId, user_id: target, action } = cmd.d ?? {};
            if (typeof channelId !== "string" || typeof target !== "string")
                return;
            if (!VOICE_ACTIONS.includes(action))
                return;
            if (voice.moderate(channelId, client.userId, target, action)) {
                announceVoice(channelId);
                // Si lo echaron de la sala, tampoco sigue en la carrera.
                if (action === "disconnect" && race.leave(channelId, target))
                    announceRace(channelId);
            }
            return;
        }
        ...
        case "VOICE_SOUND": {
            ...
            if (typeof channelId !== "string" || typeof soundId !== "string")
                return;
            ...
            for (const listener of voice.peersOf(channelId)) {
                if (voice.participantOf(channelId, listener)?.deafened)
                    continue;
        ...
        case "RACE_OPEN": {
            ...
            if (typeof channelId !== "string")
                return;
            if (race.open(channelId, client.userId))
                announceRace(channelId);
            return;
        }
        case "RACE_LEAVE": {
        ...
        case "RACE_WORLD": {
            ...
            if (typeof channelId !== "string" || typeof world !== "number")
                return;
            if (race.setWorld(channelId, client.userId, world))
                announceRace(channelId);
            return;
        }
        case "RACE_START": {
            ...
            if (race.start(channelId, client.userId))
                announceRace(channelId);
            return;
        }
        case "VOICE_SIGNAL": {
            const { channel_id: channelId, to_user_id: to, payload } = cmd.d ?? {};
            if (typeof channelId !== "string" || typeof to !== "string")
                return;
            // Reenvío ciego pero no libre: solo entre dos que ya comparten sala.
            if (!voice.canSignal(channelId, client.userId, to))
                return;
            publishToUser(to, {
                t: "VOICE_SIGNAL",
...
function writeSender(userId, into) {
    into.write(userId.replaceAll("-", ""), 1, 16, "hex");
}
function relayMedia(client, packet) {
    if (packet.length < 2)
        return;
    const kind = packet[0];
    const limit = LIMITS[kind];
    if (!limit || packet.length - 1 > limit.bytes)
        return;
    const now = Date.now();
    if (now - client.media.since >= 1000)
        client.media = { frames: 0, bytes: 0, since: now };
    if (++client.media.frames > limit.rate)
        return;
    client.media.bytes += packet.length;
    if (client.media.bytes > MAX_MEDIA_BYTES_PER_SECOND)
        return;
    ...
    if (kind === KIND_AUDIO ? sender.muted : !sender.video)
        return;
    ...
    for (const listenerId of voice.peersOf(channelId)) {
        ...
        if (kind === KIND_AUDIO && listener.deafened)
            continue;
        ...
        for (const other of targets) {
            if (other.ws.bufferedAmount > limit.buffered)
                continue;
...
export function announceRace(channelId) {
...
export function announceVoice(channelId) {
...
export function handleUpgrade(req, socket, head) {
    ...
    wss.handleUpgrade(req, socket, head, (ws) => {
        ...
        ws.on("message", (data, isBinary) => {
            if (isBinary)
                relayMedia(client, data);
            else
                handleCommand(client, String(data));
        });
        ws.on("pong", () => {
            client.alive = true;
        });
        ...
        ws.on("close", () => {
            ...
            if (!stillHere) {
                for (const channelId of voice.leaveAll(client.userId))
                    announceVoice(channelId);
                for (const channelId of race.leaveAll(client.userId))
                    announceRace(channelId);
            }
            for (const communityId of client.subs)
                broadcastPresence(communityId);
        });
    });
}
