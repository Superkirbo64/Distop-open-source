...
function downloadTarget() {
    ...
    if (platform === "win32" && arch === "x64")
        return { url: `${base}/cloudflared-windows-amd64.exe`, tgz: false };
    if (platform === "linux" && arch === "x64")
        return { url: `${base}/cloudflared-linux-amd64`, tgz: false };
    if (platform === "linux" && arch === "arm64")
        return { url: `${base}/cloudflared-linux-arm64`, tgz: false };
    if (platform === "darwin")
        return { url: `${base}/cloudflared-darwin-${arch === "arm64" ? "arm64" : "amd64"}.tgz`, tgz: true };
    return null;
}
...
async function ensureCloudflared() {
    try {
        if (spawnSync("cloudflared", ["--version"], { stdio: "ignore", timeout: 5000 }).status === 0)
            return "cloudflared";
    }
    catch {
