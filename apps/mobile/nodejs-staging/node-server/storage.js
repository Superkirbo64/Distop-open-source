...
import { HttpError, notFound, rateLimit, HANDLED } from "./http.js";
...
export function hasAudioSignature(contentType, data) {
...
export function saveUpload(opts) {
    ...
    db.prepare(`INSERT INTO attachments (id, message_id, owner_id, filename, content_type, size, path, created_at)
     VALUES (?, NULL, ?, ?, ?, ?, ?, ?)`).run(id, opts.ownerId, sanitizeName(opts.filename), opts.contentType, opts.data.length, relative, Date.now());
    return {
        id,
        message_id: null,
...
export function saveRemoteAttachment(opts) {
    ...
    const id = uuidv7();
    db.prepare(`INSERT INTO attachments (id, message_id, owner_id, filename, content_type, size, path, source_url, created_at)
     VALUES (?, NULL, ?, ?, ?, ?, '', ?, ?)`).run(id, opts.ownerId, sanitizeName(opts.filename), opts.contentType, opts.size, opts.sourceUrl, Date.now());
    return {
        id,
        message_id: null,
        filename: sanitizeName(opts.filename),
...
export function attachmentsFor(messageIds) {
    const out = new Map();
    if (messageIds.length === 0)
        return out;
    const rows = db
        .prepare(`SELECT id, message_id, filename, content_type, size FROM attachments
...
export function linkAttachments(messageId, ids, ownerId) {
    if (ids.length === 0)
        return;
    const claim = db.prepare("UPDATE attachments SET message_id = ? WHERE id = ? AND owner_id = ? AND message_id IS NULL");
    for (const id of ids)
        claim.run(messageId, id, ownerId);
}
/**
...
export async function serveFile(ctx, id) {
    const row = db
    ...
    if (row.source_url) {
        ...
        if (origen.protocol !== "https:" || !CDN_REENVIABLE.test(origen.hostname))
            throw notFound("Archivo no encontrado.");
        const res = await fetch(origen, { signal: AbortSignal.timeout(8000) }).catch(() => null);
        if (!res?.ok)
            throw new HttpError(502, "UPSTREAM_ERROR", "El archivo no se pudo traer.");
...
export function deleteStoredAttachment(id) {
    const row = db.prepare("SELECT path FROM attachments WHERE id = ?").get(id);
    if (!row)
        return;
    if (row.path) {
        const full = resolve(ROOT, row.path);
...
export function deleteAttachmentsOf(messageId) {
    const rows = db.prepare("SELECT id, path FROM attachments WHERE message_id = ?").all(messageId);
    for (const row of rows) {
 ...
 */
export function deleteAttachmentsOwnedBy(userId) {
    const rows = db.prepare("SELECT id, path FROM attachments WHERE owner_id = ?").all(userId);
    for (const row of rows) {
        if (!row.path)
            continue;
        const full = resolve(ROOT, row.path);
 ...
 */
export function purgeChatFiles() {
    const rows = db.prepare("SELECT id, path, size FROM attachments WHERE message_id IS NOT NULL").all();
    let bytes = 0;
    for (const row of rows) {
        // Reenviado (§22): sin fichero propio, path queda vacío y no hay nada que borrar.
        if (!row.path)
