...
import { canActOn, channelPermissions } from "./permissions.js";
import { getChannel } from "./entities.js";
/** canal → participantes. Vive en memoria: si la instancia cae, la llamada también. */
const rooms = new Map();
export function statesOf(channelId) {
...
export function statesOfCommunity(communityId) {
    const out = [];
    for (const channelId of rooms.keys()) {
        for (const state of statesOf(channelId))
            if (state.community_id === communityId)
                out.push(state);
    }
    return out;
}
export function peersOf(channelId) {
    return [...(rooms.get(channelId)?.keys() ?? [])];
}
/** Estado de una persona dentro de una sala, o undefined si no está. */
export function participantOf(channelId, userId) {
    return rooms.get(channelId)?.get(userId);
}
export function channelOf(userId) {
    for (const [channelId, room] of rooms)
        if (room.has(userId))
            return channelId;
    return null;
}
export function join(channelId, userId) {
    const channel = getChannel(channelId);
    if (!channel || channel.kind !== "voice")
        return null;
    if (!has(channelPermissions(channelId, userId), PERMISSIONS.CONNECT_VOICE))
        return null;
...
export function setMute(channelId, userId, muted, deafened) {
 ...
 */
export function setVideo(channelId, userId, source) {
    const participant = rooms.get(channelId)?.get(userId);
    if (!participant || participant.video === source)
        return false;
    if (source) {
        const needed = source === "screen" ? PERMISSIONS.STREAM : PERMISSIONS.USE_CAMERA;
        if (!has(channelPermissions(channelId, userId), needed))
            return false;
    }
...
export function canSignal(channelId, from, to) {
    const room = rooms.get(channelId);
 ...
 */
export function moderate(channelId, actorId, targetId, action) {
    ...
    const needed = action === "mute" || action === "unmute"
        ? PERMISSIONS.MUTE_MEMBERS
        : action === "deafen" || action === "undeafen"
            ? PERMISSIONS.DEAFEN_MEMBERS
            : PERMISSIONS.MOVE_MEMBERS;
    if (!has(permissions, needed))
        return false;
    if (!canActOn(participant.communityId, actorId, targetId))
        return false;
    switch (action) {
        case "mute":
            if (participant.forceMuted)
